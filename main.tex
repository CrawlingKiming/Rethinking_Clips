...
\citep{schulman2015trpo,schulman2017ppo}. However, recent analyses of LLM policy optimization shows that trust region solely does not determine the reliable policy optimizations. For example, recent empirical analysis reveal that less aggressive clipping thresholds \citep{yu2025dapo,park2025clipping,minimax2025m1,qi2026dppo}, even the unclipped learning can be effective. Yet, existing works lack theoretical explanations. 

Our analysis addresses a different source of error: the finite-sample gradient estimate used to make that update by linking the policy optimization to the importance-weighted estimation problem. Existing works in importance sampling reveals the central role of the ratio moments as the uncertatinty estimation error and utilize this to the off-policy estimation problem \citep{metelli2018pois,metelli2020is}. We further extend 
...
We first establish how mean squared error (MSE) in the estimated gradient limits one-step policy improvement. Under the smoothness condition on the parameterized policy, one can converts the arbitrary estimator deviation from the population gradient directly into a lower bound on policy improvement.
...
the reduction in estimation error is large enough to compensate for the useful update removed by the modification. Such preferred estimator can be determined by the comparison of the lower bound of the policy improvement.
...
Together, our analysis reveals the importance of the policy gradient reliability in policy optimization, and suggest ESS as the observable reliablity criterion. Further,  Theorem~\ref{thm:permissive-reliability} and Proposition~\ref{prop:ess-estimator-crossover} suggests the insight when other estimators can be beneficial in some cases. High ESS suggests using permissive gradient estimators, and lower ESS suggest more conservative choices such as clipping highlighting the potential benefit of stronger estimator variance control. 
...
In this section, we demonstrate our theoretical analysis in two settings. Here, we represent our result on the Gaussian bandit and LLM trajectories. 
