...
large caps or even unclipped updates, can remain stable and learn considerably faster. Yet Figure~\ref{fig:llm-motivation} shows that TIS with caps 3 and 5 can improve steadily for many
updates before abruptly collapsing. This raises the
...
of the importance weights underlying the finite-sample estimator.
...
Our analysis addresses finite-sample gradient error by linking policy optimization to importance-weighted estimation. Existing work in importance sampling reveals the central role of ratio moments in quantifying estimation uncertainty and applies this to the off-policy estimation problem \citep{metelli2018pois,metelli2020is}. We connect these
perspectives by relating importance-weight concentration to
...
where $f_\theta
 ...
 Equation~\eqref{eq:true-gradient} links policy-gradient optimization to importance-weighted mean estimation. The quality of the update depends on how reliably this finite average estimates $g(\theta)$.
...
A key quantity in importance-weighted mean estimation is the dispersion of
the likelihood weights, which can be characterized through normalized effective sample size (ESS)
...
that determines whether a finite-sample direction is useful: a positive certificate for policy optimization is obtained whenever the gradient magnitude exceeds its estimation-error radius. In practice, this error radius is unknown and requires further characterization.
...
$\rho_k:=\rho_{\theta_k}$ at update $k$. The following lemma relates this reliability radius to ESS.
...
We now extend the analysis to a general coefficient rule $u$, accounting for the bias introduced by modifying the estimator. Write the estimator MSE as $m_k(u)$. We first connect MSE to gradient-estimator reliability.
...
the reduction in estimation error is large enough to compensate for the useful update removed by the modification.
...
Together, our analysis links policy gradient reliability to policy improvement and suggests ESS as an observable reliability criterion. Theorem~\ref{thm:permissive-reliability} and Proposition~\ref{prop:ess-estimator-crossover} motivate permissive gradient estimators at high ESS and stronger variance control, such as clipping, at lower ESS.
...
We illustrate the theoretical analysis using a Gaussian bandit and LLM training trajectories.
...
\begin{figure}[htbp]
  \centering
  % Original: \includegraphics[width=0.9\linewidth]{figures/gaussian_policy_comparison.pdf}
...
\begin{figure}[htbp]
  \centering
  % Original: \includegraphics[width=0.8\linewidth]{figures_mains/motivation/ess_reliability_collapse_ess_unclipped_combine.pdf}
...
\begin{figure}[htbp]
  \centering
  % Original: \includegraphics[width=0.8\linewidth]{figures_mains/result/q30ba3b/curves/overall.pdf}
...
\begin{wrapfigure}[13]{R}{0.36\textwidth}
  ...
  \caption{Qwen3-1.7B results over extended training. Dashed black: GRPO. Red: ESS-cond. Blue: ESS for ESS-cond.}
  \label{fig:llm-17b-transfer}
  \vspace{-0.6\baselineskip}
\end{wrapfigure}
We use the reliability signal to determine the \emph{clipping timing}: retain importance-weighted updates while ESS remains above a fixed threshold and activate clipping when it falls below.
...
mathematical-reasoning objective. Figure~\ref{fig:llm-8b-transfer} extends the recipe comparison to dense Qwen3-8B. ESS-conditioned optimization demonstrates better sample efficiency than its clipped counterpart under standard GRPO, Clip-Higher, and DPPO.

\begin{figure}[htbp]
  \centering
  % Original: \includegraphics[width=0.8\linewidth]{figures_mains/result/8b/curves/overall.pdf}
  \includegraphics[width=\linewidth]
...
We developed a finite-sample analysis of policy improvement. Off-policy gradients admit an importance-weighted mean-estimation formulation, under which ESS enters directly through the second moment of the importance weights. Our theory connects this finite-sample estimation error to policy improvement and separately characterizes when clipping compensates for the gradient alignment it removes. Experiments in a Gaussian bandit and in large language models support these findings. We then extended our analysis to a simple reliability-aware optimization method, which prevents the collapse seen with the previous updates and demonstrates strong empirical performance across model sizes and objectives.
