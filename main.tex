...
\subsection{Estimated Policy gradient and improvement}

\paragraph{Policy gradient estimator}
\label{sec:raw}

\textcolor{blue}{bad writing. it is the bad flow, abrupt}
...
\paragraph{Probablistic policy improvement bounds}
...
\begin{theorem}[Policy improvement bounds]
...
\subsection{When is clipping necessary?}
\label{sec:clipping-recovery}

The preceding theorem induces a natural question: when does clipping is necessary? To answer the question, we first acknowledge that clipping can be seen as the specific type of the gradient estimator. The useful principle is to
compare the gradient estimators induced by all such rules on common policy states.

\textcolor{blue}{the term token is not very true, while our purpose is at LLM, we are not limitied to LLM. also, the definition of ratio, score, advantage can be shown first in the preliminaries..}

To formulate, let $r_{k,t}$ denote the token likelihood ratio, write
$\mathbf r_k=(r_{k,1},\ldots,r_{k,T})$, let
$s_{k,t}:=\nabla_\theta\log P_{\theta_k}(Y_t\mid X,Y_{<t})$ denote the score, and with $A_k:=R-b(X)$ being the advantage. A response-level coefficient rule is a
...
\end{equation}
We further define the bias and variance induced by the estimator:
\begin{equation}
...
\textcolor{blue}{token truncation is abrupt, we do not need this in the main text conclusion, move it to appendix this is not our "voice"}
...
\textcolor{blue}{this theorem and the previous policy imrpovement bounds, is telling different things? what's the issue? what is more improtant? if it is "an improvement" it should be stated here, if not why are we having theorem 3?}
...
\textcolor{blue}{the flow is bad.. some crude example flow:}
The preceding theorem shows when the improvement certificate can be made, when the RHS is positive, we have the policy improvement garantee ... we hearby define it as $B_k$
...
Consequently, when $0<\eta_k\le1/L_k$, the B is positive. Hence, the certifcate benefit can be seen as the .. inequality we have.. by the following collrary..
...
\section{Case Study: Linear-Quadratic regulator}

\textcolor{blue}{move miscellaneous computation to the appendix. what you should right here is, what's the purpose of this seciton, what are you going to show us.. what's the reason for this choice? is this LQR commonly used as the testbed for theory? if so cite.}
