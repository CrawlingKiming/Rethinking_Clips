...
The expectation in Equation~\eqref{eq:on-policy-gradient} is taken under the current policy $P_\theta$. For efficiency, modern policy-optimization methods often rely on reusing data \citep{schulman2017ppo, schulman2015trpo}, which creates a mismatch between the current policy and the rollout policy $Q(\cdot\mid X)$. This mismatch is then accounted for by the likelihood ratio. Formally, define the likelihood ratio and score
\begin{equation}
 r_{\theta,t}
 :=
...
Setting $u_{\mathrm{IS},t}=r_{\theta,t}$ for $t=1,\ldots,T$ corresponds to the conventional IS estimator. We consider two common estimator choices in this paper; Truncated importance sampling rule (TIS)\citep{minimax2025m1} and PPO/GRPO clippings \citep{schulman2017ppo,shao2024deepseekmath}
\begin{equation*}
...
\end{equation*}
TIS caps the corresponding ratio which reduces the magnitude of a large importance coefficient while retaining the associated score contribution. On the other hand, PPO/GRPO clipping modifies the gradient in a qualitatively different way by advantage-dependent masking
...
In this section, we provide the theoretical analysis of the finite sample policy improvement and the estimator reliablity, along with the connection with ESS. 
...
We first establish how error in the estimated gradient limits one-step policy improvement. Under the smoothness condition on the parameterized policy, one can converts the arbitrary estimator deviation from the population gradient directly into a lower bound on policy improvement.
...
We show that the error can be characterized explicitly in the case of IS estimator. At update $k$, condition on the preceding updates and write
$W_k:=W_{\theta_k}$,
$f_k:=f_{\theta_k}$,
$g_k:=g(\theta_k)$,
...
We now generalize the result to the general estimators. To faciliate the analysis, consider a general coefficient rule $u$, the same reliability question
must also account for the bias introduced by modifying the estimator. Write the MSE error of the gradient estimator as $m_k(u)$. We first connect MSE to the gradient-estimator reliablity. 
...
however, requires population MSEs and second moments. For IS, the results of
Section~4.1 allow its contribution to be expressed in terms of ESS, yielding
...
Proposition~\ref{prop:ess-estimator-crossover} isolates the role of ESS in
the crossover. All dependence of the IS certificate on importance-weight
concentration appears through $1/\rho_k$ on the left-hand side, whereas the
right-hand side collects the corresponding MSE and second-moment terms of the
alternative estimator. Conditional on these quantities, decreasing ESS makes
the inequality easier to satisfy and therefore shifts the comparison toward
estimator control. The precise crossover remains estimator dependent, which
allows truncation and clipping to become preferable at different ESS levels.
...
Proposition~\ref{prop:tis-close} shows that TIS retains the improvement certificate of untruncated importance weighting when truncation affects only a small, controlled tail.

PPO-style clipping makes a stronger modification. On the positive-advantage upper tail, TIS replaces $r_t$ by $c$ and removes only the excess $(r_t-c)_+$, whereas PPO masks the entire $r_t$-weighted contribution. PPO also masks the low-ratio negative-advantage branch. It can therefore
produce a larger reduction in estimator variation, but requires that reduction to compensate for a correspondingly larger change in the mean update.

Together, Theorem~\ref{thm:permissive-reliability} and Proposition~\ref{prop:ess-estimator-crossover} give the explanation why observable ESS values correlate with the reliable policy optimization. When ESS is relatively large, the IS estimation error is small and mild truncation can preserve the resulting importance-weighted update. As ESS decreases, the growing finite-sample error of IS increases the potential benefit of stronger variance control. 


\section{Policy optimization and ESS}
In this section, we demonstrate our theoretical analysis in two settings. Here, we represent our result on the Gaussian bandit and Large-model trajectories. 
...
\begin{figure}[htbp]
  ...
  \caption{We compare IS, TIS with $c=3$, and PPO masking with $\epsilon=0.2$. (a) Exact expected gain relative to IS; positive values favor the controlled estimator. (b) Exact expected gain around the two pairwise
...
training dynamics. All three importance-weighted updates improve substantially the early stage of training, but their performance eventually begins to degrade. At the onset of the sustained decline, all three methods exhibits low ESS. The right panel tests the theoretical prediction more directly. At each evaluation checkpoint, we pair the current ESS with the change in AIME-2024
...
importance-weighted updates are substantially less likely to produce reliable improvement. %Thus, the delayed failure observed in the introduction occurs after a substantial deterioration in the importance-weight distribution, even though the precise ESS value at which degradation begins differs across trajectories. 
...
In this section, we now extend our theorical analysis to optimization rule. Based on the observation, the reliablity signal can be used to the optimization by the \emph{clipping timing}. While ESS remains above a fixed threshold, we retain importance-weighted updates and activate clipping for a conservative updates.
...
Figure~\ref{fig:llm-main} compares ESS-conditioned optimization with static clipping across corresponding baselines. In all three cases, ESS-conditioned optimization present stronger AIME performance and efficiency. Specifically, while the TIS-3 and unclipped GRPO presents the instability, the ESS-conditioned optimization exhibits remarkable stability, preventing catastrophic collapses.
...
\begin{figure}[htbp]
  ...
  corresponds to the baseline methods. Solid curves begin with TIS-3 and switch to the corresponding clipped update when ESS falls below $0.1$. (a) GRPO clipping. (b) Clip-Higher. (c) DPPO. End labels report the final observed AIME-2024 mean@16.}
  \label{fig:llm-main}
\end{figure}
...
\end{wrapfigure}
Figure~\ref{fig:llm-noclip-gate} leaves one run
...
Together, the Figure A and B shows the empirical strength of the ESS-conditioned optimization, which supports the theoretical motivation. 

\subsection{Broader Evaluation}

\paragraph{Results on Different Scales.}
We first change model scale while retaining the
mathematical-reasoning objective. Figure~\ref{fig:llm-8b-transfer} extends the recipe comparison to dense Qwen3-8B. ESS-conditioned optimization demonstrates better sample efficiency than its clipped counterpart under both standard GRPO and Clip-Higher.
...
\begin{wrapfigure}{r}{0.45\textwidth}
  ...
  \caption{Top: Qwen3-1.7B results over extended training. AIME-2024 result is shown for GRPO (dashed) and TIS-3 + Clip (solid). Blue lined represents ESS for TIS-3 + Clip. Bottom: RLHF on Qwen3-4B-Instruct-2507. End labels report the step-200 observations.}
...
$38.5\%$, and $34.8\%$, respectively, demonstrating the collapse-prevention behavior persists across the tested range.
...
\subsection{Gaussian bandit experiment}
\label{app:gaussian}

This section provides the experimental setup and exact calculations underlying the Gaussian bandit experiments in Section~\ref{sec:gaussian-example}. All estimator moments, mean-squared errors, and expected one-step gains are computed analytically using truncated Gaussian moments. Monte Carlo simulation is used only to evaluate the batch-level probabilities reported in Figure~\ref{fig:gaussian-is-reliability}(b).
...
\subsubsection{Exact calculation of evaluation quantities}

All quantities reported in the Gaussian experiments, except for the batch-level probabilities in Figure~\ref{fig:gaussian-is-reliability}(b), are computed exactly. For each estimator, define its first two per-sample moments by
\[
...
\subsubsection{ESS and IS reliability}

Figure~\ref{fig:gaussian-is-reliability} evaluates the ESS-dependent reliability result for the IS estimator. We set
\[
N=32,
\qquad
d=2,
...
\end{align*}
Figure~\ref{fig:gaussian-is-reliability}(a) plots these two quantities against $\rho=e^{-\Delta^2}$. No Monte Carlo approximation is used in this panel.

\paragraph{Batch-level improvement experiment.}
Figure~\ref{fig:gaussian-is-reliability}(b) evaluates the corresponding batch-level policy-improvement statement. We consider 81 ESS values logarithmically spaced between $0.005$ and $0.9$. For each $\rho$, we set $\Delta=\sqrt{-\log\rho}$ and compute the theoretical error radius
\[
\varepsilon(\rho)
=
...
\]
Figure~\ref{fig:gaussian-is-reliability}(b) reports the fraction of batches satisfying the positive-certificate condition and the fraction satisfying $\Delta J>0$. These correspond to the ``positive certificate'' and ``actual improvement'' curves, respectively.
...
\subsubsection{Estimator crossover}

Figure~\ref{fig:gaussian-estimator-choice} compares IS, TIS-3, and PPO under the common fixed step size $\eta=0.4$. For the quadratic objective, the smoothness inequality is exact: for any realized scalar update $h$,
\[
J(\theta+\eta h)-J(\theta)
=
...
\end{equation}
Therefore, the zero crossings in Figure~\ref{fig:gaussian-estimator-choice}(a) are precisely the estimator crossovers predicted by the corollary.
...
\]
Figure~\ref{fig:gaussian-estimator-choice}(b) plots the exact gains $\mathcal G_{\mathrm{IS}}$, $\mathcal G_{\mathrm{T}}$, and $\mathcal G_{\mathrm{P}}$ in the neighborhood of these two crossover points. The vertical lines and open markers indicate the corresponding roots.
